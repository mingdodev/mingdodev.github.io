...
- **연결 그래프**: 모든 노드가 하나의 컴포넌트로 연결되어 있다.
- **비순환**: 사이클이 없다. 임의의 두 노드 사이 경로가 유일하다.
- **연결 + 비순환**: 노드가 N개일 때 간선은 항상 N-1개이다.

# 트리의 특징

트리 자료구조는 **계층구조**로 표현할 수 있으며, **비선형 자료구조**이다.

> **그래프와 트리의 차이?**  
> - 트리는 계층 구조(루트 노드, 부모-자식 관계)로 표현할 수 있다.
> - 그래프는 사이클이 존재할 수 있다.
