...
**루트 사용자**는 AWS 계정을 만들 때 자동으로 생성되는 기본 사용자이고, **IAM 사용자**는 AWS 계정 내에 생성된 사용자이다.

### IAM User 생성

이제 콘솔에 루트 사용자로 접속한 뒤, IAM 서비스로 IAM 사용자를 만들어 보자.

<center><img src="../../../static/img/240330/search-IAM.png" width="100%"></center>

왼쪽 상단의 검색창에서 IAM을 검색하여 접속한다.  
cf. 아래의 IAM Identify Center가 아니라 IAM으로 들어가야 한다. Identify Center를 이용하여 IAM User를 생성하고 관리할 수도 있다.
...
옵션을 선택하여 콘솔에 대한 접근 권한을 부여할 수 있다. 앞서 등장했던 Identify Center를 이용하는 것이 권장된다고는 하는데, 지금은 필요하지 않으므로 사용자 생성만 해보겠다.  
이 과정에서 콘솔에 로그인할 때 필요한 암호를 설정해준다. 이 옵션을 선택하지 않고 건너뛰어도 `사용자 정보 > 보안 자격 증명 > 콘솔 로그인`에서 콘솔 액세스를 관리할 수 있다.
...
사용자 정보에서 ID와 비밀번호를 확인하여 직접 입력하거나, 링크를 통해 접속한 뒤 비밀번호를 입력하면 로그인할 수 있다.  
콘솔에 접속해보면 루트 계정과는 달리 접근할 수 있는 서비스에 제약이 있음을 알 수 있다.
...
자동화 환경을 구축하는 경우 등 SDK/CLI를 통해 접근해야 할 때가 있기 때문에, IAM User를 통해 토큰을 발급받는 것이 필요하다!
